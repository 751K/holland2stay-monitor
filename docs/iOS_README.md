...
| 账号注销 | ✅ | 二次确认弹窗，删除 SQLite 用户配置 + 撤销 token |
...
完整移动端 API 契约见：[Backend API Reference](API.md)；机器可读 OpenAPI 3.1 契约见：[openapi.json](openapi.json)。本节保留 iOS 客户端实现视角的概要说明。
...
| DELETE | `/me` | user | — | 注销账号：删除 SQLite 用户配置 + 撤销 token |
...
├── _user_configs.py         # user_configs 表（用户配置 + App 登录字段）
...
users.py                     # UserConfig dataclass + SQLite load/save/update_users + bcrypt + 加密 + legacy users.json 迁移
```
...
- **事务写入**：用户配置写入 SQLite `user_configs`，`update_users()` 使用 `BEGIN IMMEDIATE`
- **一次性迁移**：旧 `users.json` 按 meta flag 导入 SQLite，并永久保留 `.bak` 备份
