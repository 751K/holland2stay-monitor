...
迁移之一（H2S 线上 50 次），语义是「别人的预留没成」，不是一次突兀的复活。
...
顺带解决了一个老问题：H2S 那 32 条平台报的 Reserved（最久 86 天没见到）原来
`_STALE_GENERAL_STATUSES` 不含 Reserved、孤儿路径又要求城市掉出监控，于是**永远
卡着**。现在按同一套窗口收敛。

中途试过按 (source, 状态类) 分开配阈值，最后收掉了：那些差别描述的是「feed 会不
会保留下架房源」，而实测下来四个平台的终态都是**从 feed 里消失**（H2S 227 条推测
Occupied vs 4 条平台报的，OurDomain 9 vs 1）。既然消失是共同的下架信号，就不该有
四套判据。

生产库副本上跑四轮：第 1 轮 49 条（32 条平台 Reserved 判终态 + 17 条陈旧的转推测
Reserved），第 2 轮 17 条，第 3、4 轮 0 条（幂等）。**30 分钟内刷新过的 19 条
H2S 可订全程没动。**
...
所以它库里几乎所有 Occupied 都是推测的（线上 9 推测 / 1 真实）。
...
线上因此攒了 173 条鬼影，全都在列表和地图上挂着「可订」：

| | 条数 | 最近一次见到 |
|---|---|---|
| xior Aachen Vaals | 38 | 2026-08-03 |
| h2s Rotterdam | 64 | 2026-05-08 |
| h2s The Hague | 11 | 2026-05-08 |
| h2s Utrecht | 10 | 2026-05-08 |
| 其余 16 个城市 | 50 | 05-08 ~ 05-31 |

绝大多数最后一次见到是三个月前。改一次监控城市就新增一批，而 Xior 的城市
这周已经调过两次。
...
`diff()`，所以既不写状态变更记录也不推送。线上执行后可用房源从 240 条降到
67 条，全部落在当前监控范围内。
...
整晚 102 轮里 36 轮非 6/6，**全部是同一栋楼**（Amsterdam Naritaweg，4 个房型 × 36 轮 = 144 次误判），而真正的 429 只有 8 次。
...
- **`/health` 现在 Web 和 monitor 都正常才返回 200**，否则 503。此前只看 Web 能否响应，monitor 状态仅作字段透出、不影响状态码——代价是 2026-06-13 起 monitor 停了 7 周，容器全程 `healthy`，39 个用户配置无任何通知，也没有任何告警。盲区正好盖住了唯一重要的进程。
- **判据是心跳新鲜度，不是「进程是否存在」**：
