...
* **公开页带上分享卡片**（[792f003]）
    ...
    卡片图上是真实存在过的一条房源，不是编的——og 图会被各家平台长期缓存，
    编一个不存在的地址等于把假数据钉在此后每一次分享上。

* **`/llms.txt`**（[792f003]）

    AI 检索爬虫已经是抓得最勤的一批，而它们抓到的是一张登录表单。正文里明写
    unofficial / not affiliated 并有用例钉住：被转述成官方渠道是真实风险。

### 变更

* **不带 `Accept-Language` 的客户端回落到 `en`，不再是 `zh`**（[792f003]）

    Googlebot 就是不带这个头爬的，整个站因此在被当成中文站收录。真人浏览器一定
    会发这个头，中文用户仍然协商到 zh——这条实际只影响爬虫。

* **公开页补上 `canonical` / `hreflang`，HTML 响应补 `Vary: Accept-Language`**（[792f003]）

    `/` 对匿名访客 302 到 `/login?next=/`，此前每一个 `next` 取值都是一份重复
    内容。canonical 只取路径，唯独保留 `?lang=`——那是内容真的不同的参数。

* **HTML 响应带上 `Link:` 头，Cloudflare 据此发 103 Early Hints**（[7dd5ce3]）
    ...
    Cloudflare 对 `Cache-Control: no-cache` 的动态响应同样会播 103，已实测确认。

### 修复

* **一批断言中文文案的用例一直在假绿**（[792f003]）

    它们靠「页面默认渲染中文」这个默认值，而不是自己钉住语言。最糟的一条断言
    中文文案**不出现**，页面一变英文就永远为真。另有一条数的是裸徽标文本，而英文版
    tooltip 里含同一串，徽标数被数成两倍。

* **`Vary` 合并的注释与用例都是错的**（[792f003]）
...
* Cloudflare 分析里那一批 `originResponseStatus` 为 0 的 504，是 Early Hints 子
  系统的内部记录（`requestSource: earlyHintsCache`，UA 为 `nginx-ssl` / `bastion`），
  不经过源站。按 `requestSource: "eyeball"` 统计，真实访客的 504 是 **0**。不必再查。

## v1.26.0 (2026-08-27)

三项修复，判据描述的都是另一件事：冷却时长判「是不是服务端异常」而不是「会不会
自己好」，故障上报判「整轮有没有全灭」而不是「这个代理还能不能用」，可订状态判
状态格的样式类而不是有没有下单按钮。三项都不报错——零房源、Occupied、代理健康
都是合法值。
...
* **代理冷却按故障性质分档**（[cf515a8]）
    ...
    冷却统一 600 秒，10 分钟一到就把死代理放回候选，于是每小时都有若干窗口各
    source 重新去敲一个明知欠费的代理。同一次故障在三个 source 上表现完全不同，
    取决于它们**什么时候问代理**：H2S 浏览器缓存两小时因而全程正常，Xior 每次
    重建浏览器时中招，OurCampus 每轮现取每轮中招。

    402 / 407（欠费、凭据错误——**不会自己好**）走 `PROXY_ACCOUNT_COOLDOWN_SEC`
    默认 1 小时；502 / 503（网关抽风，可能几分钟就好）保持 600 秒。只认明确的状态码，
    不并入 `X-Webshare-*` 那类错误头——从字符串里分不出账户问题还是网关问题，而判错
    的代价不对称：误判成账户级会让已恢复的代理白等一小时，反过来只是回到现状。

* **跨源隔离接住代理故障时当场上报**（[5e93545]）

    上一项修完仍有一半问题：有整段时间里 OurCampus 和 Xior 全军覆没而 H2S 一切正常，
    source 级隔离频繁触发，真正报给代理池的却寥寥无几。

    跨源隔离接住异常后就 `return` 了，而标记冷却的代码住在外层 `except ProxyError`，
    那里只有**整轮全部失败**时才够得着。于是：H2S 成功 → 整轮不算全灭 → 代理池永远
    不知道那个代理挂了 → 每轮现取代理的另外两家继续撞死代理。**H2S 的健康恰恰是把
    另外两家钉死的原因。**

    现在隔离分支里只要 `is_proxy_error(e)` 就当场上报。刻意不在这里发 admin 告警：
    外层那条已有 30 分钟节流，而这条每轮每 source 都可能触发。

* **OurDomain / OurCampus 可订判据改看下单按钮**（[064dc86]）

    OurCampus 有整批单元被报成 Occupied、一条通知都没发，而它们实际是可订的。
    原实现只看状态格第一个 span 的 class，`success` → 可订、`warning` → 抽签、
    其余一律 Occupied，而那些行是 `<span class='muted'>某个日期</span>` 配一个和
    可订单元**一模一样**的 `Book Now` 按钮。

    拿留档快照回放，`muted` 与可点的按钮严格一一对应——它的含义是「**某日起**可订」，
    不是「已出租」。

    判据改成两个独立证据取其一：状态格明确写着可订，或行里有可下单按钮。要求两者
    同时成立会把只有 `<span class='success'>` 而不带按钮的老主题整体判成不可订——
    合成 fixture 当场抓到了这个回归。没见过的样式类**放行并告警**：默默归类正是这个
    bug 的形态。

    这和 v1.24.0 里 Xior 那次是同一个错误的两个方向：那次类名说能订、文字写着
    `Rented Out`，判宽了；这次按钮明明能订、类名不认识，判严了。

    受影响的历史记录不回填——它们已经不在 feed 里，改成 Available 会发出一条指向
    不存在房源的通知。

### 备注

* v1.25.0 建的那条家庭反向 SSH 隧道当天完成常驻化（launchd + `ServerAliveInterval`，
  不用 autossh），第一个高峰即投入使用，主动降速按设计生效。

  一个坑：隧道把流量交给那台机器的网络栈，**它上面挂了 VPN 的话出口会跟着走 VPN**。
  这不是代码问题，但它让「出口是哪条线路」这个前提无声失效——降速仍然生效，实际
  出口却已经不是你以为的那个。

## v1.25.0 (2026-08-26)

**代理供应商整体失效。** 两个账号同时返回 402，代理池归零，全站停摆数小时。
代码里本来就有「降级为服务器原生 IP 直连」这条路，触发了几次，**一轮都没抓成**。
停摆的长度不是由供应商决定的，而是由下面三个自己的缺陷决定的。

### 修复

* **「降级直连」从来没有真的直连**（[8c54a00]）

    两个独立的 bug，各挡住一半。**浏览器侧**：`_open_browser` 传 `proxy=None`，但
    Chromium 绕过它自己去读 `HTTP_PROXY`，继续拨那个刚被判失效的代理。指纹是代码
    自己打出的矛盾日志——「代理层报错，但本次并未走代理」。加 `--no-proxy-server`
    之后失败耗时陡增，因为浏览器终于真的连到了 Cloudflare 并开始解挑战。

    **curl 侧**：`proxies={}` 会静默回落到环境变量，`proxies={"http": "", "https": ""}`
    才是真的直连。同一个坑 `net.py` 的注释里 v1.23.0 就写过一次，当时只修了非抓取
    路径，三处 scraper 和 `tools/doctor.py` 都漏了——doctor 尤其要紧，它正是代理
    故障时会被跑起来的工具，说「直连正常」而实际走的是死代理，会把人指向完全错误
    的方向。

* **必然失败的 source 拖垮了本来能成的**（[8c54a00]）

    在机房 IP 上逐个实测：OurDomain 与 OurCampus 直连即可，而 H2S 与 Xior 用真浏览器
    也解不开 CF 挑战。而 `dispatch_scrape_tasks` 的判据是「全员失败才上抛」，于是那
    两家的**必然失败**把 OurCampus 一起掀掉了——它本可以全程正常，实际是零。

    新增 `_apply_proxyless_gate`：代理全挂时只留实测名单里的 source。名单写死不自动
    探测——探测意味着每轮先打一次试探请求。放在节流和分片之前：被挡住的 source 不该
    白白推进分片游标。摘掉之后遥测也说实话：那两家本轮不写 `round_stats`，健康判定
    看到的是「没有这一轮」而不是「这一轮失败了」。

* **邮件配额把通知丢掉时不告警**（[1de13e5]）

    通知被 Resend 的 per-user 配额挡下这件事**只存在于日志里**——面板看不出、没有
    告警、用户更不知道，后果正是「用户以为没房源，其实是没发出去」。

    判据用**实际拒发次数**而不是「计数触顶」：一个用户正好用满之后再没房源要推给他，
    那什么都没丢。触顶是状态，拒发是事件，要报的是后者。全局用尽报 down，per-user
    触顶报 warn，一条而不是每人一条。

* **`status_changes` 上两对重复索引**（[cbb26cc]）

    `idx_sc_changed_at` ≡ `idx_status_changes_changed_at`，`listing_id` 那对同理。
    成因是 v1.3.0 的一次改名：旧名从建表代码删掉、新名用 `CREATE INDEX IF NOT EXISTS`
    建上，但没人 DROP 旧的——而 `IF NOT EXISTS` 只认名字。存量库因此长期每插一条要
    维护四个索引而不是两个。

    新增的测试守的是**这类事**而不是这两个名字：按 (表, 列组) 归并，任何一组挂两个
    就红。只比列不比名字——名字正是上次骗过所有人的东西。

### 新增

* **家庭线路作为第三顺位出口**（[8c54a00]）

    机房 IP 过不了 Cloudflare 这件事没有客户端解法——判定权重压在请求来自哪个网络。
    所以补一条不依赖供应商的出口：一台常开的 Mac 拉一条反向 SSH 隧道，
    `ssh -N -R 172.19.0.1:1080` 不带目标地址即远程动态转发，服务器上多出一个 SOCKS5。
    不需要任何额外软件，OpenSSH ≥ 7.6 即可。同一份代码只换出口，机房 IP 下解不开的
    挑战在家庭线路上几秒即过。

    几个刻意的选择：反向隧道而不是把代理端口暴露出去（公网开放代理很快会被扫到，
    家宽多半 CGNAT 也做不了入站）；绑 docker 网桥网关而不是 `0.0.0.0`，只有该网桥上
    的容器能用；不用 autossh，`ServerAliveInterval=15` + `ServerAliveCountMax=3`
    让 ssh 自己在约 45 秒内发现断链，launchd 拉起；排在池子最后——商业代理还能用时
    不该动用用户自家的 IP。最后这条不需要新代码就成立，但仍加了测试，否则哪天有人
    给池子加个排序，这条约束会静默消失。
...
* **走自家线路时主动降速**（[87b7a2b]）
    ...
    排最后只解决「什么时候用」，不解决「用多狠」：一旦落到它，节奏仍是高峰几十秒
    一轮，全打在同一个住宅 IP 上。理由不对称——商业住宅池的出口烧了换一个就是，
    **自家 IP 烧了没得换**，还连带影响家里所有上网。

    `is_personal_proxy_active()` 判定**当前在用的**是不是自家线路，不是「配没配过」。
    `PERSONAL_PROXY_MIN_INTERVAL` 默认 120 秒，两头都有边界：上界是把高峰强度压到
    约五分之一，下界是仍短于实测到的最短可订窗口，最急的房源也还有一次机会。有测试
    钉住它不得超过那个窗口。下限要夹两次——`jitter_ratio` 生产值 0.4 能把 120 秒抖
    到 72 秒，`apply_jitter` 之后必须再夹一次。

* **H2S 整批放出的抽签房源聚成一条通知**（[c946215]）

    H2S 会在同一轮里一次放出多套 `Available in lottery`，逐条发就是同一分钟内每个
    匹配用户收到十几封，配额当场被打穿。

    三条边界都是有意划的：**只限 holland2stay**（只有它会整批放）；**只限抽签状态**
    ——进抽签池不是先到先得，而「可直接预订」相反，可订窗口最短只有几分钟，少发一条
    就是少一次机会，永远逐条发；**≥2 套才聚合**，1 套时聚合与逐条等价却丢掉类型/
    楼层那几行。超过 20 套只列前 20 但抬头仍报真实总数——Telegram 单条上限 4096
    字符，列全了会被截在不可控的位置。
...
* **当前用户量对数据库设计没有压力。** 库的体积绝大部分是遥测表和它的索引，用户
  数据占比极小；每轮查询都在亚毫秒级，日志里没有出现过 `database is locked`。
  唯一随用户数线性增长的是 `update_users()` 的全量重写（读全部 → 改 → 重写所有行，
  进出都要加解密），且全程持写锁——第一堵墙在几百到一千用户之间，不是现在。
* **`api.holland2stay.com/graphql` 不是绕开代理的路。** 那个域名已经迁走，现在用的
  是 `www.holland2stay.com/api/__enc__`——同域、同一道 WAF，而且请求体 AES-GCM 加密、
  会话密钥 RSA-OAEP 包裹，密钥材料来自页面的 webpack runtime。第二道门恰恰解释了
  这个项目为什么本来就要跑浏览器。

## v1.24.0 (2026-08-25)

前五项是同一个形态：判据描述的不是它声称在描述的那件事，而且都不报错——只是安静
地给出确信的错误答案。

### 修复

* **零房源判定不再拿高频轮和全量轮相比**（[23db36f]）

    高频轮只查可订/抽签、强制 `complete=False`，看不见 Reserved；全量轮每 30 分钟
    一次才带。两者数的不是同一批房源，凑够三轮即报「抓取成功但零房源」，下一次全量
    轮一到又「已恢复」——如此反复整整一天，而其间平台上确实一套可订的都没有，抓取
    全程正常。

    改为只在 `complete > 0` 的轮次之间累积；高频零轮跳过而非打断，否则真故障永远
    凑不够三轮。这与 Xior 分片那次是同一个坑，只是切的维度是「状态」而非「楼栋」。

* **停用的 source 不再巡检**（[23db36f]）

    从面板停用一个 source 之后，它的遥测行还在保留期内，巡检照旧按行判定，于是连报
    数日「迟迟没有完整扫描」。**停用和卡死留下的痕迹一模一样**，从遥测里推不出来，
    只有配置知道。现按 `SOURCES` 判定，停用的归 `unknown` 而不是 `ok`——我们对它没有
    判断，不是判断它没事。

* **全站静默告警改按时间判定**（[23db36f]）

    `silent_rounds` 数的是「连续多少轮全站 0 条」，而分层抓取上线后零轮几乎全部是
    高频轮，同时库里有大量 Occupied 与 Reserved——「全站 0 条」正是这套设计的常态。
    换成 `HEALTH_SILENT_SECONDS`，默认 3 小时。

    **轮数在不同时段意味着完全不同的时长**，这已是第三次做同类修正（v1.20.0 完整
    扫描率、v1.21.0 限流计数）。

* **Xior 可订校验：三次连续修正**（[674a238] [9f4fecf] [27a6917]）

    这道闸门自上线起就是坏的，分三步查清。

    **取数**：文档原先写「该页面不在挑战之后，curl_cffi 直接获取即返回 200」，
    该结论不成立——生产日志复盘显示这道闸从来没有成功过，也就是说所有 Xior 通知
    走的都是未经校验的 feed 数据。改走 OurDomain 那套：浏览器风格 header、同 session
    内 403 重试（Cloudflare 首次 403 会顺手下发 `cf_clearance`）、每次尝试换出口 IP
    和 TLS 指纹，三者缺一不可。

    **判据**：RENTCafe 在户型租完后不改 `applyButton` / `ApplyNow` 属性，只改按钮
    文字（`Rented Out` vs `Available`），tile 顶上那句 `(Available)` 也不更新。改看
    文字，未见过的文字放行并告警。

    **回落**：校验打不通时旧逻辑回落信 feed，于是同一套房源在一天内被反复翻转、
    向所有匹配用户重复推送——房源本身全程未变，变的只是校验请求是否成功。新增
    `Listing.status_unverified`：scraper 只报「这次有没有依据」，怎么处置由存储层判。
    **填补空白与推翻证据是两回事**——库里已是可订就放行（这正是 fail-open 的本意），
    库里是 Occupied 而本轮无依据报可订则压住，库里没有这条则收录。代价是新单元上架
    而校验恰好不可用时，播报延后一到两轮。

* **`tenant` 的两种拼法归一**（[27a6917]）

    `student and employed` 与 `student or employed` 是同一个取值：H2S 那个 option 的
    原文用的是 or，而 `FEATURE_SYNONYMS` 只登记了荷兰语那版。后果是双份的——勾了
    「学生/上班族」的用户静默漏掉另一拼法的房源，筛选下拉还并排出现两个同义项，
    其中一个没有翻译。

### 变更

* **详情补齐缓存改为启动时从库里回填**（[c39ec6e]）

    `building_name` / `tenant_profile` 等四个字段要额外打详情接口才有，取到后记在
    进程内缓存里。缓存不落库，于是每次重启都从零开始，把已经问出过答案的房源重新
    问一遍——而部署往往一天十几次。改为启动时从库里回填，其后第一个全量轮不再产生
    多余的详情请求。

* **界面五项**（[4538adb] [3a9f416] [bc20710] [23db36f] [9f73128]）

    用户卡片操作按钮行可换行（窄列下「删除」会被裁掉，另需给 identity 补
    `min-width:0`，flex 子项默认 `min-width:auto`，长用户名会把胶囊顶出容器）；
    过滤条件勾超过四项只报个数（片区名连成无空格长串，浏览器只能在连字符处断行，
    直接溢出卡片）；自动预订三个平台各成一块，共用的申请人档案挪到两个 RENTCafe
    之后（原先夹在中间，读下来像 Xior 一直没结束）；侧栏监控圆点与图标列对齐；
    图标补深色版本——没有矢量源，直接反相会把渐变方向一起翻过来，改为先扣掉背景
    渐变与边缘辉光再用深色底重新合成（`tools/make_dark_icon.py`，可重复执行且逐字节
    一致）。

* **README 开头改成先说这是什么，再说它做不到什么**（[a0e636c]）

    原文从「荷兰租房难在哪」讲起，读到第三段才知道 FlatRadar 是个自部署的监控工具；
    免责那段也太长，「never pays on your behalf」这句要紧的被埋在解释里。
