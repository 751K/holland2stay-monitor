...
> 成功抓取，但系统完全健康。
>
> 这两个键都是**标量**，每轮被覆盖，答不了任何关于历史或分 source 的问题。
> 那类问题（「昨晚 Xior 为什么只有 2/6」）由 `round_stats` 回答。
...
打坏了，或者某个 source 一直「成功」返回 0 条。2026-06-13 起那次 7 周静默
停摆就是这一类的极端形态。

所以数据健康是**另一套判据**，落在 `round_stats` 表 + `mcore/health.py`：
...
两条「曾经有过」的前提是整套判据的关键。Xior 四栋楼常态零可订，OurCampus
官网自述排队 16–18 个月——单看「抓到 0 条」会把它们永久钉在告警上，而被无视
的告警等于没有告警。加上基线之后，规则的语义变成
**「本来有房，突然全没了」**，这正是解析器被打坏的特征。

两条基线取的位置不同，是有意的：

- 单 source 用**判定窗口内的 `max_listings`**——每个 source 的库存节奏不同，
  只能和它自己比。
- 全局用 **listings 表非空**，不用窗口。窗口只有二十来轮（约 2 小时），
  而那次静默停摆持续了 7 周；拿窗口做基线的话，故障满两小时后窗口内全是零，
  告警会自己闭嘴——恰恰在最需要它出声的时候。listings 表不会因为抓不到就
  清空（stale 收敛只改状态、不删行），是个不随故障时长衰减的基线。

**完整率用窗口平均，不用「连续不完整」的 streak。** 实测一晚 102 轮里有 36 轮
不是 6/6——partial 在这个系统里是常态，streak 规则会持续误报。代价是最近几轮
的骤降会被历史稀释（20 轮 6/6 + 4 轮 2/6 仍是 89%，不触发），换来的是告警不会
被噪音淹掉。要看骤降就看 `/monitoring` 的轮次表格。

**数据退化不改 `/health` 的状态码。** 重启治不好解析器对不上，只会打断正在
进行的抓取。退化只告警、只上面板。

告警由 `mcore/watchdog.py` 每轮巡检一次，只发 admin。两个设计点：

- **节流状态写 meta，不放内存。** supervisor 的 autorestart 恰恰会在故障时
  频繁重启，节流放内存等于最该节流的时候失效。
- **恢复也发通知。** 只报警不报恢复，等于逼人继续 ssh 上去确认好了没有。

---

## 8. 排障入口

先开 `/monitoring`（admin）——分 source 健康卡 + 最近 30 轮明细 + 当前活跃告警。
下面这张表里大部分问题它能直接回答，不用 ssh。
