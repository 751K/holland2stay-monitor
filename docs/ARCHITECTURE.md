...
自动预订的关键路径由两段成本拼成，量级差一个数量级：

| 段 | 属于 | 实测 | 失败过吗 |
|---|---|---|---|
| 过 Cloudflare 挑战 | **浏览器** | 中位 16.5s、p90 18.9s（480 次） | 有记录的两次下单失败全死在这 |
| NextAuth 登录 | 用户 | ~1.5s | 未见 |

`try_book` 的每个调用都用 `Authorization: Bearer <JWT>` 认证，浏览器只是一条过了
CF 的 HTTP 通道，与登录者无关。也就是说贵的、会失败的那一半可被所有用户共用。

2026-09-08 之前不是这么做的：预登录仅在「本轮出现候选」时启动，而候选的定义是
「房源已变为 Available」——预登录与抢房因此在同一瞬间开始，每次都要现过一次 CF。
留存日志中快速通道触发 4 次、`prewarmed=no` 4 次、成功 0 次；两次有记录的失败
（2026-08-27 代理 402、2026-09-08 CF 熔断）均未走到实际下单。

`mcore/warm_browser.py` 常驻一个已过挑战的浏览器，由 `monitor` 每轮心跳维护，将
那 16.5 秒的不确定性移出关键路径——它在后台按自身节奏重试，房源翻牌时关键路径只
剩 1.5 秒的登录。

约束与代价：

- **保活是必需项而非优化。** `cf_clearance` 的标称有效期无效，实际生效的是同域
  `h2s_clr`，寿命 0.5 小时；浏览器持续发请求时由服务端续期，闲置则退化为空壳。
  心跳每 10 分钟发一次该 profile 自己的 clearance 探针，保活与健康检查合一。
- **仅共用浏览器，不共用登录。** 「无候选的轮次完全不触碰 H2S 登录接口」这一约束
  原样成立，登录仍只发生在候选出现时。
- **常驻只借给本轮优先级最高的那个用户**，其余用户照旧各自开浏览器。

  不采用「排队共用」：该队列在此改动之前并不存在（每用户各开一个浏览器、完全
  并行），且 `threading` 的锁唤醒任意等待者，既非 FIFO，更不遵循 `sort_order`。
  用户排序是用户在设置页明确配置的，不应被锁的唤醒顺序改写。现行做法下第一名
  走 1.5 秒的暖路径，其余与改动前完全一致——无人变差。

  `user_notifiers` 本身即 `sort_order` 序（`list_user_config_rows` 的 `ORDER
  BY`），故「循环中第一个」即「优先级最高者」；跨轮仍持有常驻的缓存 session 由
  `PrewarmCache.lane_in_use()` 识别。

  锁仍然保留：单个 `BrowserFetcher` 不可多线程并用，而跨轮换借用者时短暂出现两
  个 session 同时引用它是可能的。那是安全兜底，不是常态路径。
- **借来的一律不关。** `PrewarmedSession.owns_fetcher` 区分二者。此处出错是静默
  的：关闭全进程共用的浏览器不抛异常、不进日志，下一位使用者只是拿到空壳。
...
| 位置 | 作用 |
|---|---|
| `scrapers/<平台>.py` 的 `CITIES` | 决定「站点出现未登记城市」的 WARNING 报不报 |
| `config.KNOWN_*_CITIES` | 决定设置页上能勾哪些（**菜单**） |
| `app_settings` 里的订阅串 | 决定实际抓哪些（**点的菜**） |

2026-09-08 Plaza 上架 Rijswijk 时这条链子完整暴露了一次。前两处已合并为一处
（`scrapers/plaza.py` 的 `CITIES` 从 `KNOWN_PLAZA_CITIES` 派生）；第三处不能合并
——它是用户的选择，不是事实。

于是产生一个静默失败：把城市加进注册表**只是让它出现在菜单上**。订阅串是明确列举
的，新增项不在其中，因此 WARNING 因注册表已包含它而停止，抓取因订阅串未包含它而
照旧不发生——唯一的痕迹被修复动作本身抹除。

`target_config.registry_drift()` 补上这一格，`monitor` 启动时调用。判据是**增量**
而非集合包含：仅报告「与上次所见注册表相比新出现、且未进入订阅」的条目。集合包含
不可用——实测生效配置中 holland2stay 26 城订阅 2 个、Xior 30 栋订阅 4 栋，均为明确
选择，按集合包含判据首轮即产生 50 条报告，其中 0 条为真。

两处刻意的设计：

- **未处理的新增不写回快照。** 报告一次即记入快照的话，重启一次警告便永久消失，
  而问题仍在。留在快照之外，则每次启动重复报告，直至该城市被勾选。
- **设置页保存视为确认。** 保存时用户面前即是整张菜单，新城市是页面上的一个复选
  框，其提交即为选择。此后不再提醒（`target_config.ack_registry()`）。

快照存于 `meta` 表的 `registry_targets_seen`，非 `app_settings`——后者仅装 runtime
配置，混入其他键会使 `hydrate()` 每次启动报「不属于 runtime 类」。
...
### 4.2 从 feature 派生的排序列

`area` 与 `energy` 在 `features` 里是**文本**——`"87.28 m²"` 与 `"B"`。按文本排序
得到的是字典序：`"9 m²"` 排在 `"87.28 m²"` 之后，`"A+++"` 排在 `"A"` 之前。故落成
两列（`mstorage/_derived.py`）：

| 列 | 含义 | 方向 |
|---|---|---|
| `area_value` | 平方米 | 越大越大 |
| `energy_rank` | `ENERGY_LABELS` 下标 | **越小越好**，`A+++` = 0 |

两列可空且无默认值：`NULL` 表示不知道（抓不到面积、能耗标签不在白名单内）。给
默认 0 会让这些行挤在最优的一端，而 0 是合法的面积与等级，看上去像真值。

**它们与 `features` 是同一事实的两种形态，因此必须同写。** `mstorage` 中共五处写
`features`：`diff()` 的一个 INSERT 与两个 UPDATE，以及两处改写 features 的迁移。
只写 INSERT 的后果是「新数据正确、存量永远为空」——与 2026-09-05 `os_version` 那次
同形。测试以静态扫描兜底：任何写 `features=?` 的 SQL 附近若不含 `area_value`，判定
失败。

两列不出现在 API 响应中：客户端已有 `features` / `feature_map`，再给一份即是同一
事实的第二个副本。它们只服务于服务端排序。

### 4.2b feature 取值的归一与匹配
...
- **OurCampus**：`OurCampusBooker` 已写好但**未注册**。流程与 OurDomain 相同，
  ...
  `Join Lottery` 不开始申请。未验证的是端到端流程，以及「开始申请能否占住单元」
  这一前提本身——验证方法见 `tools/ourcampus_booker_probe.py`。
