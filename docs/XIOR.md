...
`RATE_LIMIT_BACKOFF` 负责。退避后仍失败时，处置分三层：

1. 该栋楼标记为 incomplete；
2. **同一批次内剩余的楼栋直接判定为失败，不再发出请求**——429 是针对出口 IP 的
   配额判定，剩余楼栋必然同样被拒，逐个重试只是各自睡满一轮退避去重新证明一件
   已知的事，实测每次爆发因此阻塞整轮约 6 分钟；
3. 触发 Xior 的 source 级熔断（10 分钟起，上限 1 小时），冷却到期后以 1 栋楼作
   canary 试探。

三层均只作用于 Xior，其余平台同轮照常抓取。详见
[ARCHITECTURE.md §5.5 与 §5.6.1](ARCHITECTURE.md)。
