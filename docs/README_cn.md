...
| Android App | Beta | Kotlin + Compose，功能完整（57 文件，~9.5k 行），FCM 推送已验证，Play Store 准备中 |
| 桌面版本 | 可用 | GitHub Releases 提供 macOS `.dmg` 和 Windows `.zip` |

[Download on the App Store](https://apps.apple.com/us/app/flarradar/id6769857080) · [下载 Android App](https://github.com/751K/holland2stay-monitor/releases/latest/download/app-release.aab)

## 快速开始

### Docker

Docker 推荐用于 VPS 或长期运行的家用服务器。**最低 1 GB 内存**（CloakBrowser ~190 MB + Python/Gunicorn）。
...
# 安装 CloakBrowser patched Chromium（H2S Cloudflare 绕过，~300MB）
...
> **macOS 注意**：CloakBrowser macOS 版使用稍旧的 Chromium（v145，26 patches vs Linux v146，58 patches）。生产环境建议使用 Docker（Linux）。
