...
所有 `GET` 200 JSON 响应自动附带 `ETag` 和 `Cache-Control` 头，客户端可用 `If-None-Match` 做条件请求：

| 响应头 | 值 | 说明 |
|---|---|---|
| `ETag` | `"<md5>"` | 响应体的 MD5 哈希，内容不变则 ETag 不变 |
| `Cache-Control` | `private, max-age=10, must-revalidate` | 10s 新鲜窗口，超窗后自动带 `If-None-Match` 复验（默认） |
| `Cache-Control`（`/notifications`） | `private, no-cache` | 每次必须复验（通知已读状态高频变化），但仍支持 304 |

行为：

- 客户端发 `GET /api/v1/listings` 带 `If-None-Match: "<上次的etag>"`：
  - 内容未变 → **304 Not Modified**，无 body，节省带宽
  - 内容已变 → **200** + 新 body + 新 ETag
- 10s 内重复请求无需网络（`max-age=10`），对移动端 Tab 切换（列表 ↔ 地图 ↔ 日历）特别有效
- iOS 端用 `URLCache`（2MB 内存 + 20MB 磁盘）自动处理；Android 端用 OkHttp 默认缓存
...
| POST | `/admin/monitor/restart` | admin | 重启监控进程 |
...
| `area` | 面积（㎡），派生列 `area_value` |
| `energy` | 能耗等级排名，派生列 `energy_rank`，**升序 = 最好的在前**（A+++ → F） |
| `first_seen` | 首次发现时间 |
| `last_seen` | 最近一次见到 |
...
**四、`area` 和 `energy` 走派生列。** 它们在 `features` 里存的是文本
（`"87.28 m²"` / `"B"`），按文本排是字典序——`"9 m²"` 会排到 `"87.28 m²"` 后面，
`"A+++"` 会排到 `"A"` 前面。所以库里另存了两列：

| 列 | 含义 | 方向 |
|---|---|---|
| `area_value` | 平方米 | 越大越大 |
| `energy_rank` | `A+++ A++ A+ A B C D E F` 的下标 | **越小越好**，`A+++` = 0 |

这两列**不出现在响应里**——客户端已经有 `features` / `feature_map`，再给一份等于
同一个事实两个副本。它们只用于服务端排序。

抓不到面积、能耗标签不在白名单里（上游冒出没见过的写法）时写 `NULL`，不写 0：
0 是一个合法的面积和等级，用它表示"不知道"会让那些房子挤在最优的一端，比排在
最后更糟，因为它看起来像真值。
...
### POST `/admin/monitor/restart`

重启 monitor 进程（先 stop 再 start）。
...
Android 客户端已接入全部接口，FCM 推送已端到端拉通：
