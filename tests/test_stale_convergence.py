...
**从 feed 里消失**，H2S 227 条推测 Occupied vs 4 条平台报的，OurDomain 9 vs 1。
既然消失是共同的下架信号，就不该有四套判据。
...
这件正事第一段就办到了），而 ``Reserved → 可订`` 本来就是最常见的正常迁移之一
（H2S 线上 50 次），语义是「别人的预留没成」，不是一次突兀的复活。

线上凭据（2026-08-04）
----------------------
- OurDomain 房源寿命 0.2–3.1 小时（一批出现，一条条被订走），而原阈值 7 天；
- H2S 抽签：从抽签出去的状态迁移总共 3 次，其余都是直接消失；当时 4 条抽签
  房源已经消失 31–44 小时还挂着「可抽签」；
- H2S 平台报的 Reserved 有 32 条，最久 86 天没见到。
...
class TestSecondStage:
    ...
    def test_a_reserved_still_in_the_feed_is_untouched(self, store):
        ...
        H2S 的 Reserved 常态就长：线上配对出的 51 段里中位 6.3 小时、75 分位
        28 小时、最长 21 天，而且绝大多数最后回到「可订」。只要它还在 feed 里
        被列出，last_seen 每轮刷新，这条路径就永远够不着它。
        """
