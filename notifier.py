...
class BaseNotifier(ABC):
    ...
    async def send_new_listings_batch(self, listings: list[Listing]) -> bool:
        ...
        H2S 的抽签不急：进抽签池不是先到先得，晚看半小时不影响结果。可直接
        预订的房源相反，仍然逐条发。**这是 H2S 的结论，不是抽签的通性**——
        OurCampus 的 Join Lottery（2026-09-15 #2301）挂 48 分钟就撤了，所以
        它照样逐条发，见 ``monitor._split_batchable``。
        """
        if not listings:
            return False
